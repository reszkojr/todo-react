...
export default function Sidebar() {
	const { todos, setActiveTodo, createTodo, activeTodo, isSidebarOpen, toggleSidebar } = useTodo();
	...
	return (
		<div className='flex'>
			<div className={`flex flex-col w-10/12 h-screen bg-background-800 md:relative md:w-1/4 fixed top-0 left-0 z-50 p-4 transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
						...
						<div key={status} className='mb-4'>
							...
							<div className='space-y-2 ml-3'>
								{todos
									...
									.map((todo) => (
										<div key={todo.id} className='p-2 bg-background-500 rounded cursor-pointer hover:bg-gray-600' onClick={() => handleTodoClick(todo)}>
				...
				<button className='absolute bottom-0 left-0 w-full p-4 cursor-pointer bg-background-700 bg-opacity-75 backdrop-blur-md text-white flex items-center justify-center space-x-2' onClick={handleCreateTodo}>
					<LuPlus size={18} />
