	...
	useEffect(() => {
		...
		if (isEditing) {
            if (dragButtonRef.current) {
                dragButtonRef.current.classList.add('hidden');
            }
			document.addEventListener('mousedown', handleClickOutside);
		} else {
            if (dragButtonRef.current) {
				dragButtonRef.current.classList.remove('hidden');
			}
		...
		<div ref={setNodeRef} style={style} className='relative p-4 mb-2 bg-background-500 w-[320px] rounded shadow group flex items-center justify-between min-h-20 max-h-50'>
			<div ref={todoRef} className='flex items-center justify-between w-full'>
				<div>
					{isEditing ? (
						<>
							<input type='text' autoFocus value={title} onChange={(e) => setTitle(e.target.value)} className='bg-background-100 rounded-md border-1 border-background-200 w-11/12 font-bold' />
							<textarea value={description} onChange={(e) => setDescription(e.target.value)} className='bg-background-100 rounded-md border-1 border-background-200 line-clamp-6 w-11/12 mt-3 h-25 max-h-28' />
						</>
					) : (
						<>
				...
				<button {...listeners} {...attributes} ref={dragButtonRef} className='cursor-grab mr-2 mix-blend-soft-light'>
