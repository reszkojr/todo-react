...
const TextInput: React.FC<TextInputProps> = ({ label, type, name, register, requiredMessage, errorMessage, className = '', labelClassName = '', inputClassName = '', errorClassName = '' }) => {
	return (
		<div className={className}>
			...
			<input type={type} {...register(name, { required: requiredMessage })} className={`w-full px-3 py-2 mt-1 border rounded-sm shadow-sm bg-gray-700 border-gray-600 text-white focus:outline-none focus:ring focus:ring-indigo-500 ${inputClassName}`} />
