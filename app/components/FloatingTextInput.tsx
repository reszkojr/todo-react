...
const FloatingTextInput: React.FC<FloatingTextInputProps> = ({ label, type, register, requiredMessage, errorMessage, className = '', labelClassName = '', inputClassName = '', errorClassName = '' }) => {
	return (
		<div className={`relative ${className}`}>
			<input type={type} {...register(label.toLowerCase(), { required: requiredMessage })} className={`block px-2.5 pb-2.5 pt-5 w-full rounded-sm text-sm bg-gray-700 border-0 border-b-2 appearance-none text-white border-gray-600 focus:outline-none focus:ring-0 focus:border-teal-600 peer ${inputClassName}`} placeholder=' ' />
